...
// DATAS
// Formata datas exibidas na interface como 01.set.2026.
export function formatarData(valor) {
  return formatarDataCurta(valor);
}

// Formata uma string "YYYY-MM-DD" ou objeto Date para o padrão 01.set.2026.
export function formatarDataCurta(valor) {
  ...
  const mes = data.toLocaleDateString("pt-BR", { month: "short" }).replace(/\.$/, "").toLowerCase();
  return `${dia}.${mes}.${data.getFullYear()}`;
}

export function formatarDataSemAno(valor) {
  return formatarDataCurta(valor).replace(/\.\d{4}$/, "");
}
...
export function formatarTelefone(valor) {
  ...
  // Remove código do país (55) se vier com DDI
  if (numeros.length > 11) {
    numeros = numeros.slice(-11); // pega os últimos 11 dígitos
  }
...
// Calcula o horário de fim somando minutos ao inicio
export function calcularFim(inicio, duracaoMinutos) {
  if (!inicio || !duracaoMinutos) return "";
  ...
  return `${data}T${String(fimH).padStart(2, "0")}:${String(fimM).padStart(2, "0")}`;
}

export function agruparPorMes(atendimentos) {
  const grupos = {};
  atendimentos.forEach((a) => {
    const data  = new Date(a.inicio);
    const chave = `${data.getFullYear()}-${String(data.getMonth() + 1).padStart(2, "0")}`;
    const label = data.toLocaleDateString("pt-BR", { month: "long", year: "numeric" });
    if (!grupos[chave]) grupos[chave] = { label, itens: [] };
...
export function somarServicos(servicos = []) {
  return servicos.reduce((acc, s) => acc + parseFloat(s.valor ?? 0), 0);
}
